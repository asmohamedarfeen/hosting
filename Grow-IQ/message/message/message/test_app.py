#!/usr/bin/env python3
"""
Test script for the LinkedIn-like Connection + Messaging App.
Demonstrates user registration, connections, and messaging functionality.
"""
...
def test_user_registration():
    """Test user registration and get tokens."""
    ...
    if response.status_code == 201:
        ...
        print(f"❌ Failed to register Alice: {response.text}")
        return None, None
    ...
    response = requests.post(f"{BASE_URL}/users/register", json=user2_data)
    if response.status_code == 201:
        user2_token = response.json()["access_token"]
        print("✅ Bob registered successfully")
    else:
        print(f"❌ Failed to register Bob: {response.text}")
        return None, None
...
def main():
    """Main test function."""
    print("🚀 LinkedIn-like Connection + Messaging App - Test Suite")
    print("=" * 60)
