...
export const LoginPage = (): JSX.Element => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        // Backend expects form fields: identifier, password, remember
        const form = new FormData();
        form.append("identifier", formData.email);
        form.append("password", formData.password);
        // Optionally support remember later
        ...
        if (!res.ok) {
          ...
          try {
            ...
            setErrorMessage(json.detail || json.message || "Login failed");
          } catch {
        ...
        setLocation(data.redirect_url || "/home");
        return;
      }

      // Signup flow -> map to backend fields
      const form = new FormData();
      // Map UI "User ID" to backend-required unique username
      form.append("username", formData.userId);
      form.append("email", formData.email);
      form.append("full_name", `${formData.firstName} ${formData.lastName}`.trim());
      form.append("company", "");
      form.append("password", formData.password);
      ...
      if (!res.ok) {
        ...
        try {
          const json = JSON.parse(text);
          setErrorMessage(json.detail || json.message || "Signup failed");
        } catch {
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!isLogin && (
                <div className="space-y-2">
                  <Label htmlFor="userId" className="font-caption">User ID (unique)</Label>
                  <Input
                    id="userId"
                    required
                    ...
                    className="border-[#673ab733] focus:border-[#673ab7]"
                    placeholder="Choose a unique ID (e.g., arfeen123)"
                  />
                  <p className="text-xs text-neutral-500">This is different from your name and must be unique.</p>
                </div>
              )}
              ...
              <Button
                type="submit"
                disabled={submitting}
                className="w-full bg-[#673ab7] hover:bg-[#673ab7]/90 text-white py-3 disabled:opacity-60"
              >
