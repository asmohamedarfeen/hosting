...
// Company data - will be fetched from API
const sampleCompanies: any[] = [];

export const CompanyProfilePage = (): JSX.Element => {
  const [match, params] = useRoute("/company/:id");
  const [location, setLocation] = useLocation();
  
  // Company data should be fetched from API based on companyId
  const companyId = params?.id ? parseInt(params.id) : null;
  const company = sampleCompanies.find(c => c.id === companyId) || null;
  ...
  // If no company found, show message
  if (!company) {
    return (
      <div className="bg-neutral-100 min-h-screen flex items-center justify-center">
        <Card>
          <CardContent className="p-8 text-center">
            <h2 className="font-h5 text-[#673ab7] mb-4">Company Not Found</h2>
